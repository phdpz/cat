	...
	function select(){
  		...
  		if(from>to)
  			alert("开始时间必须大于等于结束时间");
  		 	...
  		 	window.location.href="${pageContext.request.contextPath}/analyse/analyse?school="+school+"&area="+area+"&from="+from+"&to="+to;
  	}
	</script>
	<% 
		Date d = new Date(); 
			...
			<div class="content">
				...
				<div class="container">
					...
					<ul class="report_nav" id="report_nav">
						...
						<li><a href="<%=request.getContextPath() %>/analyse/analyse" class="active">品种分析</a></li>
						<span id="line"></span>
					</ul>
						    ...
						    <table class="table" id="tableSort">
								<thead>
									<tr name="head">
										...
										<th>营业额
											<span class="fa fa-caret-up caret-up" onclick="javascript:$.sortTable.sort('tableSort',3,0)"></span>
											<span class="fa fa-caret-down caret-down" onclick="javascript:$.sortTable.sort('tableSort',3,1)"></span>
										</th>
										<th>营业额比例
											<span class="fa fa-caret-up caret-up" onclick="javascript:$.sortTable.sort('tableSort',3,0)"></span>
											<span class="fa fa-caret-down caret-down" onclick="javascript:$.sortTable.sort('tableSort',3,1)"></span>
								...
								<tbody>
									<c:forEach items="${goodsList}" var="good">
							  			<tr>
							  				<td>${good.location}${good.fruitType}</td>
							  				<td>${good.totalWeight}</td>
							  				<td><fmt:formatNumber type="percent" minFractionDigits="2">${good.radio}</fmt:formatNumber></td>
							  				<td>${good.totalFee}</td>
							  				<td><fmt:formatNumber type="percent" minFractionDigits="2">${good.radio1}</fmt:formatNumber></td>
							  			</tr>
							  		</c:forEach>
								</tbody>
							</table>
...
<html>
  ...
  <br><br>
	...
	<a href="<%=request.getContextPath()%>/fruit/manageFruit">返回</a>
		...
		学校：<select name="school" id="school" onChange="queryArea(this.value)">
				<option>全部</option>
				<c:forEach items="${stationList}" var="school1">
					<option <c:if test="${school==school1}">selected=selected</c:if>>${school1}</option>
				</c:forEach>
	...
	<img src="${typeChart1}"/>
  	<table border="1px" id="tableSort">
  		...
  		<tr>
  			<th>品种</th>
  			<th>总销售量<a href="javascript:$.sortTable.sort('tableSort',1,0)">▲</a><a href="javascript:$.sortTable.sort('tableSort',1,1)">▼</a></th>
  			<th>百分比<a href="javascript:$.sortTable.sort('tableSort',1,0)">▲</a><a href="javascript:$.sortTable.sort('tableSort',1,1)">▼</a></th>
  			<th>营业额<a href="javascript:$.sortTable.sort('tableSort',3,0)">▲</a><a href="javascript:$.sortTable.sort('tableSort',3,1)">▼</a></th>
  			<th>百分比<a href="javascript:$.sortTable.sort('tableSort',3,0)">▲</a><a href="javascript:$.sortTable.sort('tableSort',3,1)">▼</a></th>
  		</tr>
  		</thead>
  		<c:forEach items="${goodsList}" var="good">
  			<tr>
