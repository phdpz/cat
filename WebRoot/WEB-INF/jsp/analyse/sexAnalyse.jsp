...
<html>
	...
	function checkDate()
	  ...
	  function search()
	  {
		 ...
		 var urlStr = "<%=request.getContextPath()%>/analyse/sexAnalyseAjax/"+school+"/"+area+"/"+beginDate+"/"+endDate;
			$.ajax({
				method: "GET",
				url: urlStr,
				success:function(data,status,jqXHR){
					document.getElementById("boy").innerHTML=data.boy;
					document.getElementById("boyRadio").innerHTML=data.boyRadio;
					document.getElementById("girl").innerHTML=data.girl;
					document.getElementById("girlRadio").innerHTML=data.girlRadio;
					document.getElementById("img").src="<%=request.getContextPath()%>/DisplayChart?filename="+data.fileName;
					
					document.getElementById("boyMoney").innerHTML=data.boyMoney;
					document.getElementById("boyMoneyRadio").innerHTML=data.boyMoneyRadio;
					document.getElementById("girlMoney").innerHTML=data.girlMoney;
					document.getElementById("girlMoneyRadio").innerHTML=data.girlMoneyRadio;
	   ...
	   function getAreas(school)
	   {
		   if(school!='全部'){
				var urlStr = "<%=request.getContextPath()%>/admin/getAreas1/"+school;
				$.ajax({
					method: "GET",
					url: urlStr,
					success:function(data,status,jqXHR){
			...
			<div class="content">
				...
				<div class="container">
					...
					<ul class="report_nav" id="report_nav">
						...
						<li><a href="<%=request.getContextPath()%>/analyse/sexAnalyse/全部/全部/2015-08-01/<%=date%>" class="active">性别分析</a></li>
					...
					<div class="panel">
						...
						<div class="panel_body">
							<span class="setSpace">学校:</span>
								<select class="input school"id="school" onChange="getAreas(this.value)">
         							<option>全部</option> 
								</select>
							<span class="setSpace">站点:</span>
								<select class="input school"id="area">
         							<option>全部</option>
								</select>	
						    ...
						    <table class="table" id="sex">
								<thead>
									<tr name="head">
										<th>学校</th>
										<th>下单量
											<span class="fa fa-caret-up caret-up" onclick="javascript:$.sortTable.sort('sex',1,0)"></span>
											<span class="fa fa-caret-down caret-down" onclick="javascript:$.sortTable.sort('sex',1,1)"></span>
										</th>
										<th>下单量比例
											<span class="fa fa-caret-up caret-up" onclick="javascript:$.sortTable.sort('sex',1,0)"></span>
											<span class="fa fa-caret-down caret-down" onclick="javascript:$.sortTable.sort('sex',1,1)"></span>
										...
										<th>营业额
											<span class="fa fa-caret-up caret-up" onclick="javascript:$.sortTable.sort('sex',3,0)"></span>
											<span class="fa fa-caret-down caret-down" onclick="javascript:$.sortTable.sort('sex',3,1)"></span>
										</th>
										<th>营业额比例
											<span class="fa fa-caret-up caret-up" onclick="javascript:$.sortTable.sort('sex',3,0)"></span>
											<span class="fa fa-caret-down caret-down" onclick="javascript:$.sortTable.sort('sex',3,1)"></span>
								...
								<tbody>
							        ...
							        <td id="boy">${boy}</td>
							        <td id="boyRadio">
							         <c:if test="${(boy+girl)!=0}">
							          <fmt:formatNumber type="PERCENT" value="${boy/(boy+girl)}" maxFractionDigits="2"/>
							         </c:if>
							         <c:if test="${(boy+girl)==0}">
							           0%
							         </c:if>
							        </td>
							        <td id="boyMoney">${boyMoney}</td>
							        <td id="boyMoneyRadio">
							         <c:if test="${(boyMoney+girlMoney)!=0}">
							          <fmt:formatNumber type="PERCENT" value="${boyMoney/(boyMoney+girlMoney)}" maxFractionDigits="2"/>
							         </c:if>
							         <c:if test="${(boyMoney+girlMoney)==0}">
							           0%
							         </c:if>
							      ...
							      <tr align="center">
							        ...
							        <td id="girlRadio">
							         <c:if test="${(boy+girl)!=0}">
							          <fmt:formatNumber type="PERCENT" value="${girl/(boy+girl)}" maxFractionDigits="2"/>
							         </c:if>
							         <c:if test="${(boy+girl)==0}">
							           0%
							         </c:if>
							        </td>
							        <td id="girlMoney">${girlMoney}</td>
							        <td id="girlMoneyRadio">
							         <c:if test="${(boyMoney+girlMoney)!=0}">
							          <fmt:formatNumber type="PERCENT" value="${girlMoney/(boyMoney+girlMoney)}" maxFractionDigits="2"/>
							         </c:if>
							         <c:if test="${(boyMoney+girlMoney)==0}">
							           0%
							         </c:if>
...
<html>
  ...
  %>
    ...
    <br><br><br>
   <font style="font-size:20px;font-weight:bold">性别统计</font> 
   
   <br><br>
         学校：<select id="school" onChange="getAreas(this.value)">
         <option>全部</option>  
       </select>
         站点：
       <select id="area">
