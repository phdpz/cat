...
public class sexChartUtil {
	private static PieDataset getPieDataset(Map<String, Object> map){
		//创建一个饼图的数据集
		DefaultPieDataset dataset = new DefaultPieDataset();
		if(map.containsKey("boy"))
		{
			dataset.setValue("男", (Integer)map.get("boy"));
			dataset.setValue("女", (Integer)map.get("girl"));
		}
		if(map.containsKey("boyMoney"))
		{
			dataset.setValue("男", (Double)map.get("boyMoney"));
			dataset.setValue("女", (Double)map.get("girlMoney"));
		}
		return dataset;
	}
	...
	public static JFreeChart getJFreeChart(Map<String, Object> map){
		...
		if(map.containsKey("boy"))
		  chart=ChartFactory.createPieChart("水果销量性别分析", dataset,true,true,false);
		if(map.containsKey("boyMoney"))
		  chart=ChartFactory.createPieChart("水果营业额性别分析", dataset,true,true,false);
		setPiePlotNum(chart);
		//设置背景图片
