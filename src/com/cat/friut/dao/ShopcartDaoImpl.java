...
public class ShopcartDaoImpl extends BaseDaoImpl<Shopcart> implements ShopcartDao{
	...
	protected Class<Shopcart> entityClass = (Class<Shopcart>) GenericsUtils.getSuperClassGenricType(this.getClass());
	//
	protected static String tableName = "Shopcart";
	// 配置组件，方便方法的使用
	/**
	 * @Description 获取记录总数
	 * @return int
	...
	public List<Shopcart> getListByOpenId(String openId){
		return this.sessionFactory.getCurrentSession().createQuery("select from "+tableName+" where openId='"+openId+"'").list();
	}
	
	public List<Object[]> getListByOpenIdGroupByType(String openId){
		return this.sessionFactory.getCurrentSession().createQuery("select f.FId,f.FGodsName,f.FFruitType,s.SNum,f.FRealFee,f.FWeight,f.FLocation from Shopcart as s,Fruitgoods as f where openId='"+openId+"' and s.fruitId = f.FId order by f.FFruitType")
				.list();
	}
	
	public List<Object[]> getPrices(String openId){
		String hql= "select s.SNum,f.FRealFee from Shopcart as s,Fruitgoods as f where s.openId='"+openId+"' and s.fruitId = f.FId";
	...
	public List<Shopcart> findByProperty(String propertyName, Object value) throws HibernateException{
		...
		return (List<Shopcart>) this.sessionFactory.getCurrentSession().createQuery(hql).setParameter(0, value).list();
	}
	/**
	...
	public List<Shopcart> findAll(final LinkedHashMap<String, String> orderBy) throws Exception{
		...
		return (List<Shopcart>) this.sessionFactory.getCurrentSession().createQuery("from " + tableName +" o " + buildOrderby(orderBy)).list();
	}
	/**
	...
	public QueryResult<Shopcart> listByPage(final int page,final int size){ 
        ...
        final QueryResult<Shopcart> queryResult = new QueryResult<Shopcart>();
        //获取总记录数
